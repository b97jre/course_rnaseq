...
<p class="large danger""> THIS TOPIC NEEDS TO BE REVIEWED </p>
...
We will clone this Github repo from NBIS annotation team which will be used during this pratical.
...
fastqc ~/assembly/assembly_annotation/raw_computes/ERR305399_1.fastq.gz -o fastqc/
```
...
```{sh,block.title=TRUE,eval=FALSE}
scp user@rackham.uppmax.uu.se:/home/user/RNAseq_annotation/report.html .
```
...
```{sh,block.title=TRUE,eval=FALSE}
export PERL5LIB=$PERL5LIB:~/assembly/GAAS/annotation/

~/assembly/GAAS/annotation/Tools/bin/fastq_guessMyFormat.pl -i ~/assembly/assembly_annotation/raw_computes/ERR305399_1.fastq.gz
```
...
```{sh,block.title=TRUE,eval=FALSE}
java -jar /sw/apps/bioinfo/trimmomatic/0.36/milou/trimmomatic-0.36.jar PE -threads 5 -phred33 ~/assembly/assembly_annotation/raw_computes/ERR305399_1.fastq.gz ~/assembly/assembly_annotation/raw_computes/ERR305399_2.fastq.gz trimmomatic/ERR305399.left_paired.fastq.gz trimmomatic/ERR305399.left_unpaired.fastq.gz trimmomatic/ERR305399.right_paired.fastq.gz trimmomatic/ERR305399.right_unpaired.fastq.gz ILLUMINACLIP:/sw/apps/bioinfo/trimmomatic/0.36/milou/adapters/TruSeq3-PE.fa:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36
```
...
hisat2-build ~/assembly/assembly_annotation/chromosome/chr4.fa index/chr4_index
```
...
```{sh,block.title=TRUE,eval=FALSE}
scp __YOURLOGIN__@rackham.uppmax.uu.se:/home/__YOURLOGIN__/assembly/RNAseq_assembly/stringtie/transcripts.gtf .
```
...
Trinity --seqType fq --max_memory 32G --left ~/assembly/assembly_annotation/raw_computes/ERR305399_1.fastq.gz --right ~/assembly/assembly_annotation/raw_computes/ERR305399_2.fastq.gz --CPU 5 --output trinity --SS_lib_type RF
```

Trinity takes a long time to run (like hours), you can stop the program when you start it and have a look at the results, look in ~/assembly/assembly_annotation/RNAseq/trinity the output is Trinity.fasta
...
/sw/apps/bioinfo/trinity/2.4.0/rackham/util/misc/process_GMAP_alignments_gff3_chimeras_ok.pl --genome ~/assembly/assembly_annotation/chromosome/chr4.fa --transcripts ~/assembly/assembly_annotation/RNAseq/trinity/Trinity.fasta > gmap/transcript_trinity.gff
```
...
/sw/apps/bioinfo/BUSCO/3.0.2b/rackham/bin/run_BUSCO.py -i ~/assembly/assembly_annotation/RNAseq/trinity/Trinity.fasta -o busco_trinity -l $BUSCO_LINEAGE_SETS/arthropoda_odb9 -m tran -c 5
```

Busco will take 30 to run so you can check the results in `~/assembly/assembly_annotation/RNAseq/busco_trinity`
...
export PERL5LIB=$PERL5LIB:~/assembly/GAAS/annotation/
~/assembly/GAAS/annotation/Tools/bin/gff3_sp_extract_sequences.pl --cdna -g transcripts.gtf -f ~/assembly/assembly_annotation/chromosome/chr4.fa -o transcripts_stringtie.fa
...
Compare the two BUSCO results, what do you think happened for StringTie?
