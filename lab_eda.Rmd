...
metadata <- read.csv2("./data/metadata_raw.csv",row.names=1)

#Convert ensembl IDs to gene names
annot <- read.delim("./data/human_genes.txt",header=T)
...
logcpm <- log2( t( round(t(data) / colSums(data) * 1e6)) + 1)
```
...
boxplot( t(logcpm[ top_var[1:30],])   , ylim=c(0,12), las=2 , col="grey" , main="logCPM (top var genes)" ,cex=.2)
```
...
The usefulness of PCA is that the principal components do have a meaning: They store the amount of variance in dereasing order, so some PCs are more important than others. Inside the `PC$sdev` object, we can get the standard deviation stored in each PC.
...
* What happens if instead of using the formula above, we used the absolute value of $r$ ( $|r|$ ) as adjacency (this way all values will be also between 0-1). Does this change affects the interpretation of adjency distances ?
...
gene_clusters <- cutree(gene_clus, h = 0.5)
gene_clusters
```
...
Side Note: always keep in mind that the bootstrapping procedure is very time consuming and computationally intensive.

```{r,results='hide',block.title=TRUE,fig.height=5,fig.width=10,eval=FALSE}
mypar(1,2,mar=c(6,4,2,1))
#Clustering on the MOCK dataset
mock <- matrix(rep(logcpm[1:100,1],12),byrow = F,ncol = 12)
mock <- mock + rnorm(n = 12*nrow(logcpm),mean = 0,sd = .01)
...
plot(mock_pvc,las=2,hang = -0.000001)

#Clustering on our dataset
pvc <- pvclust( data = logcpm , method.dist = "correlation", method.hclust = "complete",parallel = T,r=1)
