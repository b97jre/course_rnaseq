...
```{r}
boxplot(log10(as.matrix(cr)+1),ylab="Log10 (Read counts)",las=2,
        main="Raw data")
```
...
```{r}
boxplot(log10(as.matrix(cf)+1),ylab="Log10 (Read counts)",las=2,
        main="Filtered data")
```
...
```{r}
cc <- edgeR::cpm(cf)
boxplot(log10(as.matrix(cc)+1),ylab="Log10 (Read counts)",las=2,main="CPM")
```
...
```{r}
ct <- tpm(cf1,g1$len)
boxplot(log10(as.matrix(ct)+1),ylab="Log10 (Read counts)",las=2,main="TPM")
```
...
boxplot(log10(as.matrix(cd)+1),ylab="Log10 (Read counts)",las=2,main="DESeq2")
```
...
boxplot(log10(as.matrix(cv)+1),ylab="Log10 (Read counts)",las=2,main="VST")
```
...
The data is converted to a DESeq2 object. The GLM model we use is simple since we only have one variable of interest `~Time`. If we had other covariates to control for, we would add them to the model like so `~var+Time`. The variable of interest appears in the end.
...
For those interested in the details of the *median-of-ratios* method, it is described below.
...
ggplot(data.frame(mean=log10(dm+1),var=log10(dv+1)),
  ...
  labs(x="Log10 Mean counts",y="Log10 Variance")+
  theme_bw()
```

We see a mostly linear relationship on the log scale. The blue line denotes a linear fit. Genes that have larger read counts show higher variance. It's hard to say which genes are more variable based on this alone. A measure that controls for this mean-variance relationship is what we need.
...
ggplot(data.frame(mean=log10(dm+1),var=dc),
  ...
  geom_point(alpha=0.2)+
  geom_smooth()+
  labs(x="Log10 Mean counts",y="Coefficient of variation")+
  theme_bw()
```
...
For the sake of completeness, we can also plot the relationship between CV and variance for the same sample groups.
...
ggplot(dx3,aes(x=log10(var+1),y=cv))+
  ...
  geom_smooth()+
  facet_wrap(~sample.x)+
  labs(x="Log10 Variance",y="Coefficient of variation")+
  theme_bw()
```
